...
def generar_pdf_detalles_pedido(pedido_id, cliente, fecha_entrega, horario_entrega, metodo_pago, monto, descuento, pagado, productos, cantidades, precios, direccion, telefono, observaciones):
    pdf_path = f"orden_pedido_{pedido_id}.pdf"
    LOGO_PATH = Config.LOGO_PATH
    total_final = 0
    ...
    total_final = total_precio - descuento
    elements.append(Paragraph(f"Descuento: -${descuento:,.2f}", styles["Normal"]))
    elements.append(Spacer(1, 10))
    elements.append(Paragraph(f"Total: ${total_final:,.2f}", styles["Normal"]))
    elements.append(Spacer(1, 10))
